...
class ManuallyProvidedMacHostKeyVerifier extends MacHostKeyVerifier implements ServerHostKeyVerifier {

    private static final Logger LOGGER = Logger.getLogger(ManuallyProvidedMacHostKeyVerifier.name)

    private final MacHostKey parsedKey

    @DataBoundConstructor
    ManuallyProvidedMacHostKeyVerifier(final String key) {
        super()
        try {
    ...
    static final class DescriptorImpl extends Descriptor<MacHostKeyVerifier> {
        ...
        public String getDisplayName() {
            return "Manually provide host key"
        }
        ...
        public FormValidation doCheckKey(@QueryParameter String key) {
            try {
                ManuallyProvidedMacHostKeyVerifier.parseKey(key)
